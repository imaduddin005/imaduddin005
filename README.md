...
Optimized ML models through hyperparameter tuning and PCA-based dimensionality reduction; applied transfer learning across ResNet, DenseNet, EfficientNet, and YOLOv9 to accelerate development on limited datasets; implemented LSTM/GRU variants to handle long-range dependencies in sequential tasks.

**AI Engineering Intern** — Comrex Pvt Ltd · *Jul 2024 – Sep 2024*
Built end-to-end **RAG pipelines** using LangChain — integrating vector databases and document embeddings to deliver context-aware, hallucination-resistant LLM responses. Developed multi-step reasoning workflows using **LangChain agents** for complex question-answering and task automation. Designed **Agentic AI** systems enabling LLMs to autonomously call external tools and APIs. Integrated these into production-style chatbots and knowledge-based assistants.

**AI Intern** — Ministry of IT & Telecom, Pakistan · *Aug 2023 – Oct 2023*
Conducted data preprocessing and EDA on structured government datasets; built Python-based visualizations (Pandas, NumPy, Matplotlib) to surface trends and KPIs for digital transformation initiatives.
...
#### 🎙️ [Voice-Based Personal Assistant](https://github.com/imaduddin005)
`LangChain` `Whisper` `Agentic AI` `Tool Integration`
A voice-driven AI assistant combining **OpenAI Whisper** for speech recognition with **LangChain agents** for tool-augmented, multi-turn reasoning. The assistant autonomously integrates external APIs and tools to execute tasks and hold real-time conversational interactions — agentic AI applied to a hands-free interface.

#### 🐔 [Vaccine Combo Prediction for Poultry](https://github.com/imaduddin005)
`Scikit-learn` `Ensemble Methods` `PCA` `Target Encoding`
A complete ML pipeline predicting optimal vaccine combinations for poultry, using ensemble models, model-based imputation for missing veterinary data, and target encoding for categorical features. Applied **PCA** for dimensionality reduction to reduce noise and improve generalization on structured, real-world agricultural data.
